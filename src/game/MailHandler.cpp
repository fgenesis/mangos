...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
    ...
    // FG: do not allow to have more than 100 mails in mailbox.. mails count is in opcode uint8!!! - so max can be 255..
    if (mails_count > 100)
    {
    ...
    if (items_count > 0 || money > 0)
    {
        if (items_count > 0)
        {
            ...
            {
                Item* item = items[i];
                if (GetSecurity() >= SEC_MODERATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
                {
        ...
        if (money > 0 &&  GetSecurity() >= SEC_MODERATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE))
        {
