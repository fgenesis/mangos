...
public:
    ...
    {
        i_percent = (uint32)unit->GetHealthPercent();
    }
    Unit* getUnit() const { return i_unit; }
    uint32 getPercent() const { return i_percent; }
private:
...
WorldObject* Spell::FindCorpseUsing()
{
    ...
    Cell::VisitGridObjects(pCaster, searcher, max_range);
...
void Spell::AddUnitTarget(Unit* pVictim, SpellEffectIndex effIndex)
{
    ...
    // FG: added later
    if(m_spellInfo->AttributesEx3 & SPELL_ATTR_EX3_CANT_MISS)
        canMiss = false;
...
void Spell::DoAllEffectOnTarget(TargetInfo *target)
{
    ...
    else if (m_damage)
    {
        ...
        // Divine Storm (use m_healthLeech to store damage for all targets) // FG: CHECK THIS
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (m_spellInfo->SpellFamilyFlags.test<CF_DRUID_STARFALL2>())
                unMaxTargets = 2;
            // Wild Growth
            else if (m_spellInfo->IsFitToFamilyMask(UI64LIT(0x0400000000000000)))
                unMaxTargets = 5;
            break;
        }
    ...
    switch(targetMode)
    {
        ...
        case TARGET_ALL_FRIENDLY_UNITS_IN_AREA:
            ...
            // Glyph of Holy Light
            else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_GENERIC && m_spellInfo->SpellIconID == 963)
            {
                Unit* target = m_targets.getUnitTarget();
                if(!target)
                    target = m_caster;
            ...
            // Echoes of Light (Althor's Abacus)
            else if (m_spellInfo->SpellFamilyName == SPELLFAMILY_GENERIC && m_spellInfo->SpellIconID == 2542)
            {
                Unit* target = m_targets.getUnitTarget();
                if(!target)
                    target = m_caster;

                FillRaidOrPartyHealthPriorityTargets(targetUnitMap, m_caster, target, radius, 1, true, false, true);
            }
        ...
        case TARGET_DYNAMIC_OBJECT_RIGHT_SIDE:
        {
            ...
            if(m_spellInfo->EffectImplicitTargetB[effIndex] == TARGET_AREAEFFECT_GO_AROUND_DEST)
            {
                //FIXME
                break;
            }
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            // Mirror Image (glyph)
                ...
                AddPrecastSpell(65047);                     // Mirror Image (summon 4th image)
            // Icy Veins
...
{
    // FG: HACK to disable some spell effects
    switch(m_spellInfo->Id)
    {
        case 1038: if(i == 0) return; break;
    }
    // FG: -end-
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_DUMMY:
            {
                ...
                {
                    if(!m_targets.getUnitTarget() || (m_targets.getUnitTarget()->GetHealthPercent() > 20 && !m_caster->IsIgnoreUnitState(m_spellInfo, IGNORE_UNIT_TARGET_STATE)))
                        return SPELL_FAILED_BAD_TARGETS;
                }
            ...
            case SPELL_EFFECT_SCHOOL_DAMAGE:
            {
                ...
                if (m_spellInfo->IsFitToFamily<SPELLFAMILY_PALADIN, CF_PALADIN_HAMMER_OF_WRATH>())
                {
                    ...
                    if (m_targets.getUnitTarget()->GetHealthPercent() > 20)
                        return SPELL_FAILED_BAD_TARGETS;
                }
                break;
            }
    ...
    if (spellInfo->ManaCostPercentage)
    {
        switch (spellInfo->powerType)
        {
            ...
            case POWER_HAPPINESS:
                powerCost += caster->GetPercentOfMaxPower(Powers(spellInfo->powerType), float(spellInfo->ManaCostPercentage));
                break;
            case POWER_RUNE:
    ...
    for(int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        switch (m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC:
            {
                ...
                // FG: + extra check to prevent enchanting heirloom items. Not sure if its correct, but i added this ages ago and noone complained.
...
bool Spell::IsTargetVisibleAndInLOS(void) const // FG: for spell channeling out of sight interruption
{
    if(Unit *target = m_targets.getUnitTarget())
    ...
    return true; // all other cases -- too error prone to check all possible other cases
}
