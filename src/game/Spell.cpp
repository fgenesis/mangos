...
{
    // FG: compile fix, force caster to be player
    ...
    T u_check((const Player*)m_caster, max_range);
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    // FG: added in ff27a51e308b9be3756024004befbaf66f180066, not sure if this is still needed
    bool reqCombat=true;
    Unit::AuraList const& stateAuras = m_caster->GetAurasByType(SPELL_AURA_IGNORE_UNIT_STATE);
    for (Unit::AuraList::const_iterator j = stateAuras.begin(); j != stateAuras.end(); ++j)
    {
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_DUMMY:
            {
                ...
                {
                    if(!m_targets.getUnitTarget() || m_targets.getUnitTarget()->GetHealthPercent() > 20 && !m_caster->isIgnoreUnitState(m_spellInfo))
                        return SPELL_FAILED_BAD_TARGETS;
                }
    ...
    if (m_spellInfo->ManaCostPercentage)
    {
        switch (m_spellInfo->powerType)
        {
            ...
            case POWER_HAPPINESS:
                powerCost += m_caster->GetPercentOfMaxPower(Powers(m_spellInfo->powerType), float(m_spellInfo->ManaCostPercentage));
                break;
            case POWER_RUNE:
