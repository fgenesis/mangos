...
Player::Player (WorldSession *session): Unit(), m_mover(this), m_camera(this), m_achievementMgr(this), m_reputationMgr(this)
{
    ...
    // FG: old movement anticheat - left for reference!
...
void Player::Update( uint32 update_diff, uint32 p_time )
{
    ...
    // FG: HACKFIX - make dead players really dead, prevent dead-but-alive bug
    // R2: (fg) - added GHOULED check
...
void Player::GiveXP(uint32 xp, Unit* victim)
{
    ...
    //prevent Northrend Level Leeching :P (// FG: LOL WTF)
    ...
    // Heirloom Experience Bonus (// FG: this is no longer needed, right?)
...
void Player::UpdateFreeTalentPoints(bool resetIfNeed)
{
    ...
    else
    {
        ...
        {
            if (resetIfNeed && GetSession()->GetSecurity() < SEC_MODERATOR)
                resetTalents(true);
            else
                SetFreeTalentPoints(0);
        }
...
bool Player::RewardHonor(Unit *uVictim, uint32 groupsize, float honor)
{
    ...
    if (honor <= 0)
    {
        ...
        else
        {
            ...
            // FG: creature extended custom honor values...
            const CreatureExtendedInfo *exinfo = cVictim->GetCreatureExtendedInfo();
            if(exinfo && exinfo->honor)
            {
                honor =  exinfo->honor;
            }
            else // FG: not custom case ...
            {
        ...
        if (pSource->GetTypeId() == TYPEID_UNIT)
        {
            ...
            switch(itr->second.option_id)
            {
                ...
                case GOSSIP_OPTION_LEARNDUALSPEC:
                    if(!(GetSpecsCount() == 1 && pCreature->CanTrainAndResetTalentsOf(this) && !(getLevel() < sWorld.getConfig(CONFIG_UINT32_MIN_DUALSPEC_LEVEL))))
                        hasMenuItem = false;
                    break;
        ...
        case GOSSIP_OPTION_LEARNDUALSPEC:
            if(GetSpecsCount() == 1 && !(getLevel() < sWorld.getConfig(CONFIG_UINT32_MIN_DUALSPEC_LEVEL)))
            {
                ...
                else*/
                {
                    ...
                    this->CastSpell(this,63680,true,NULL,NULL,this->GetObjectGuid());
                    this->CastSpell(this,63624,true,NULL,NULL,this->GetObjectGuid());
...
void Player::RewardQuest(Quest const *pQuest, uint32 reward, Object* questGiver, bool announce)
{
    ...
    uint32 xp = uint32(pQuest->XPValue(this) * GetSession()->GetXPMultiQuest()); // FG: we use custom multi here!
...
void Player::KilledMonsterCredit( uint32 entry, ObjectGuid guid )
{
    ...
    for( int i = 0; i < MAX_QUEST_LOG_SIZE; ++i )
    {
        ...
        Quest const* qInfo = sObjectMgr.GetQuestTemplate(questid);
        if (!qInfo || !qInfo->CanBeAccounted(this))
            continue;
...
{
    if (m_clientGUIDs.empty() || !GetMap(false))
        return;
...
void Player::ActivateSpec(uint8 specNum)
{
    ...
    // FG: Remove temp enchantments (like poisons or shamans flametongue) at SpecSwitch (- from andeeria)
    RemoveAllEnchantments(TEMP_ENCHANTMENT_SLOT);
