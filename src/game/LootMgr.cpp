...
void Loot::AddItem(LootStoreItem const & item)
{
    ...
    if (item.needs_quest)                                   // Quest drop
    {
        ...
        for(std::vector<LootItem>::iterator it = m_questItems.begin(); it != m_questItems.end(); it++)
...
bool Loot::FillLoot(uint32 loot_id, LootStore const& store, Player* loot_owner, bool personal, bool noEmptyError, uint32 minloot)
{
    // Must be provided
    if(!loot_owner)
        return false;
