        ...
        case SPELLFAMILY_GENERIC:
        {
            switch(GetId())
            {
                ...
                case 43354: // Q: Seeds of the Blacksouled Keepers
                    ...
                    {
                        if( target->GetTypeId() == TYPEID_UNIT && target->GetEntry() == 23876 )
...
/* // FG: kept for reference, not sure how to port this without using SpellModifier
        case SPELLFAMILY_PRIEST:
        {
            // Pain and Suffering
            ...
            // Borrowed Time
            if( GetSpellProto()->SpellIconID == 2899 && target->GetTypeId()==TYPEID_PLAYER )
            {
                if(apply)
                {
            ...
            switch(GetId())
            {
                ...
                case 53563:
                {
                    if(apply)
                        ...
                        target->CastSpell(target,53651,true,NULL,this,target->GetObjectGuid());
                    else
                        target->RemoveAurasByCasterSpell(53651,target->GetObjectGuid());
                    return;
                }
                ...
                case 57958:
                {
                    ...
                    if (apply)
                    {
                        ...
                        mod->spellId = GetId();
                        mod->mask.Flags = UI64LIT(0x2);
                        mod->mask.Flags2= UI64LIT(0x0);
                        m_spellmod = mod;
                    }
                ...
                case 57979:
                {
                    ...
                    if (apply)
                    {
                        ...
                        mod->type = SPELLMOD_FLAT;
                        mod->spellId = GetId();
                        mod->mask.Flags = UI64LIT(0x10000); // taken from family flags, not sure if its correct
                        mod->mask.Flags2= UI64LIT(0x0);
                        m_spellmod = mod;
                    }
...
*/
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            /* // FG: FIXME: no idea right now how to fix this after spell modifiers were removed...
            if( GetSpellProto()->SpellIconID == 57 && target->GetTypeId()==TYPEID_PLAYER )
            {
                if(apply)
                {
                    ...
                    switch (m_effIndex)
                    {
                        case 0:
                            // Windfury Totem
                            mod->mask.Flags = UI64LIT(0x00200000000);
                            mod->mask.Flags2= UI64LIT(0x0);
                            break;
                        case 1:
                            // Flametongue Totem
                            mod->mask.Flags = UI64LIT(0x00400000000);
                            mod->mask.Flags2= UI64LIT(0x0);
                            break;
                    }
            ...
            else if((GetSpellProto()->SpellIconID == 338 || GetId() == 55456) && target->GetTypeId()==TYPEID_PLAYER)
            {
                if (apply)
                {
                    SpellModifier *mod = new SpellModifier;
...
void Aura::HandleModMechanicImmunity(bool apply, bool /*Real*/)
{
    ...
    // Berserk
    else if(GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID && GetSpellProto()->SpellFamilyFlags.Flags2 & 0x40)
    {
        if (apply)
            target->CastSpell(target, 58923, true, 0, this);
        else
            target->RemoveAurasDueToSpell(58923);
    }
}
    ...
    if (apply && !loading)
    {
        switch (GetId())
        {
            ...
            case 61782:                                     // Infinite Replenishment (0.25% from max)
               m_modifier.m_amount = target->GetPercentOfMaxPower(POWER_MANA, 0.25f);
                break;
            default:
                break;
        }
...
void Aura::HandlePeriodicDamage(bool apply, bool Real)
{
    ...
    else
    {
        ...
        else if (spellProto->Id == 54361 || spellProto->Id == 59743)    // Void Shifted 
        ...
        if (GetEffectMechanic(GetSpellProto(), GetEffIndex()) == MECHANIC_BLEED)
        {
            ...
            if (!found)
                target->ModifyAuraState(AURA_STATE_BLEEDING, false);
        }
    }
}
...
void Aura::HandleAuraModResistanceExclusive(bool apply, bool /*Real*/)
{
    for(int8 x = SPELL_SCHOOL_NORMAL; x < MAX_SPELL_SCHOOL;x++)
    {
        ...
        if (m_modifier.m_miscvalue & int32(1<<x))
        {
            // FG: nos4r2zod aura stacking (see b560d2717305adf88a3d5e0a11fa5b40d6049479 for reference)
...
/*{
    ...
    for(int8 i = SPELL_SCHOOL_NORMAL; i < MAX_SPELL_SCHOOL; i++)
    {
        ...
        {
            // FG: old code by nos4r2zod - (aura stacking patch?)
            ...
            {
                target->ApplyResistanceBuffModsPercentMod(SpellSchools(i), m_positive, change, apply);
            }
        }
    }
}
...
void Aura::HandleModTotalPercentStat(bool apply, bool /*Real*/)
{
    ...
    if (apply && target->GetTypeId() == TYPEID_PLAYER && target->getClass() == CLASS_DRUID && target->GetShapeshiftForm() && m_modifier.m_miscvalue == STAT_STAMINA
        ...
        && m_spellAuraHolder->GetSpellProto()->SpellFamilyFlags.Flags2 & 0x00000002)
    {
...
void  Aura::HandleAuraModIncreaseMaxHealth(bool apply, bool /*Real*/)
{
    ...
    float healthPercentage = target->GetHealthPercent();
    ...
    if (oldhealth > 0)
    {
        uint32 newhealth = uint32(target->GetPercentOfMaxHP(healthPercentage));
        if (newhealth==0)
            newhealth = 1;
...
void SpellAuraHolder::SendFakeAuraUpdate(uint32 auraId, bool remove)
{
    ...
    {
        data.appendPackGUID(GetCasterGuid().GetRawValue());
    }
