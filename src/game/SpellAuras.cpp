...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleIgnoreUnitState,                           //262 SPELL_AURA_IGNORE_UNIT_STATE Allows some abilities which are avaible only in some cases.... implemented in Unit::isIgnoreUnitState & Spell::CheckCast                                     //262 ignore combat/aura state?
        ...
        case SPELLFAMILY_PALADIN:
        {
            switch(GetId())
            {
                ...
                case 53563:
                {
                    if(apply)
                        // original caster must be target (beacon)
                        target->CastSpell(target,53651,true,NULL,this,target->GetGUID());
                    else
                        target->RemoveAurasByCasterSpell(53651,target->GetGUID());
                    return;
                }
...
void Aura::HandleModMechanicImmunity(bool apply, bool /*Real*/)
{
    ...
    // Berserk
    else if(GetSpellProto()->SpellFamilyName == SPELLFAMILY_DRUID && GetSpellProto()->SpellFamilyFlags2 & 0x40)
    {
        if (apply)
...
void Aura::HandlePeriodicDamage(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        switch (spellProto->SpellFamilyName)
        {
           ...
           {
               // Drain Soul
               if (GetSpellProto()->SpellIconID == 113)
               {
    ...
    if(Unit* caster = GetCaster())
    {
        ...
        if (apply)
        {
            switch(GetId())
            {
                // Fingers of Frost
                case 44544:
                    holder->SetAuraCharges(3); // 3 because first is droped on proc
                    break;
                ...
                case 64976:
                case 57499:
                    holder->SetAuraSlot(255);
                    holder->SetAuraFlags(19);
