...
bool ChatHandler::HandleMuteCommand(char* args)
{
    ...
    if(uint32 maxtime = sWorld.getConfig(CONFIG_UINT32_MAX_MUTETIME))
    {
...
bool ChatHandler::HandlePInfoCommand(char* args)
{
    ...
    QueryResult* result = LoginDatabase.PQuery("SELECT username,gmlevel,last_ip,last_login,joindate,email FROM account WHERE id = '%u'",accId);
    if (result)
    {
        Field* fields = result->Fetch();
