...
void Spell::EffectDummy(SpellEffectIndex eff_idx)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            // FG: is this still needed? i think not, i read they fixed it elsewhere
            /*switch(m_spellInfo->Id)
            {
                // Death Grip
                case 49560:
...
void Spell::EffectTriggerMissileSpell(SpellEffectIndex effect_idx)
{
    ...
    // FG: triggered missile spells on cooldown is wrong imho. fixes freezing arrow at least.
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
...
void Spell::EffectEnchantItemPerm(SpellEffectIndex eff_idx)
{
    ...
    if (item_owner!=p_caster && p_caster->GetSession()->GetSecurity() >= SEC_MODERATOR && sWorld.getConfig(CONFIG_BOOL_GM_LOG_TRADE) )
    {
...
void Spell::EffectScriptEffect(SpellEffectIndex eff_idx)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
            ...
            switch(m_spellInfo->Id)
            {
                ...
                case 48025:                                 // Headless Horseman Mount
                {
                    ...
                    case 150:
                        ...
                        return;
                    case 225:
                        if ((((Player*)unitTarget)->CanStartFlyInArea(((Player*)unitTarget)->GetMapId(), zone, area) && area != 4395 && ((Player*)unitTarget)->GetMapId() == 571) || ((Player*)unitTarget)->GetMapId() == 530)
        ...
        /*case SPELLFAMILY_WARRIOR: // FG: left here for reference, handled below
        {
            // Shattering Throw
            if (m_spellInfo->Id == 64380)
            {
                ...
                for(Unit::SpellAuraHolderMap::iterator iter = Auras.begin(), next; iter != Auras.end(); iter = next)
                {
                    ...
                    if(GetAllSpellMechanicMask(proto) & (1 << (MECHANIC_IMMUNE_SHIELD-1)))
                    {
                        unitTarget->RemoveAurasDueToSpell(proto->Id);
                        if(Auras.empty())
                            break;
                        else
                            next = Auras.begin();
        ...
        case SPELLFAMILY_WARRIOR:
        {
            switch(m_spellInfo->Id)
            {
                ...
                case 64380:                                 // Shattering Throw
                {
                    ...
                    m_caster->CastSpell(unitTarget, 39897, true); // FG: eh.. is this correct? mass dispel?
                    break;
                }
            }
            break;
