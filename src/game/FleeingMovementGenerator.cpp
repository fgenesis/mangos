...
FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    if( owner.hasUnitState(UNIT_STAT_ROOT | UNIT_STAT_STUNNED | UNIT_STAT_DIED) ) // FG: maybe also | UNIT_STAT_ON_VEHICLE
        return;
...
FleeingMovementGenerator<T>::Update(T &owner, const uint32 & time_diff)
{
    if( !&owner || !owner.isAlive() )
        return false;
    if( owner.hasUnitState(UNIT_STAT_ROOT | UNIT_STAT_STUNNED | UNIT_STAT_DIED) ) // FG: mabe also UNIT_STAT_ON_VEHICLE
        return true;
