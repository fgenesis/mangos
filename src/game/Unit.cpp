...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    if (health <= damage)
    {
        ...
        if (player_tap != pVictim && bRewardIsAllowed)
        {
            if (group_tap)
...
void Unit::CalculateMeleeDamage(Unit *pVictim, uint32 damage, CalcDamageInfo *damageInfo, WeaponAttackType attackType)
{
    ...
    switch(damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_GLANCING:
        {
            ...
            {
                case CLASS_SHAMAN:
                    if (GetTypeId() == TYPEID_PLAYER && GetInt32Value(UNIT_FIELD_ATTACK_POWER) < SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) &&
                        GetInt32Value(UNIT_FIELD_ATTACK_POWER) < SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_MAGIC))
                        iscaster = true;
                    break;
                case CLASS_DRUID:
                    if (IsInFeralForm())
...
void Unit::DealMeleeDamage(CalcDamageInfo *damageInfo, bool durabilityLoss)
{
    ...
    // FG: process rage generation here, and not in DealDamage()
    if(this != pVictim && GetTypeId() == TYPEID_PLAYER && getPowerType() == POWER_RAGE)
    {
        ((Player*)this)->RewardRageForDamageDealt(damageInfo, &cleanDamage);
    }
        ...
        switch(spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (spellProto->SpellIconID == 2135)
                {
                    ...
                    int32 absorbableDamage = RemainingDamage + 0.35f * pCaster->GetMaxHealth() - pCaster->GetHealth();
                    ...
                    if (!((Player*)pCaster)->HasAura(66233))
                        preventDeathSpell = (*i)->GetSpellProto();
        ...
        switch(preventDeathSpell->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_PALADIN:
            {
                ...
                if (preventDeathSpell->SpellIconID == 2135)
                {
                    ...
                    int32 defenseAmount = pCaster->GetDefenseSkillValue() - pCaster->getLevel() * 5;
                    ...
                    if (defenseAmount > 0)
                    {
                        ...
                        pCaster->CastSpell(pCaster, 66233, true);
                        // Calculate heal amount
                        int32 healAmount = preventDeathSpell->CalculateSimpleValue(EFFECT_INDEX_1);
                        healAmount = defenseAmount * pCaster->GetMaxHealth() * healAmount / 14000 - pCaster->GetHealth();
                        // Heal if positive value
                        if (healAmount > 0)
                            pCaster->CastCustomSpell(pCaster, 66235, &healAmount, NULL, NULL, true);
                        // Absorb Everything
                        RemainingDamage = 0;
                    }
                }
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell, bool canMiss)
{
    ...
    if(canMiss)
    {
        ...
        // Roll miss
        //uint32 tmp = spell->AttributesEx3 & SPELL_ATTR_EX3_CANT_MISS ? 0 : missChance; // FG: checked elsewhere
        if (roll < tmp)
...
bool Unit::IsImmuneToSpell(SpellEntry const* spellInfo)
{
    ...
    if (spellInfo->SpellFamilyName == SPELLFAMILY_WARRIOR && spellInfo->SpellFamilyFlags.Flags == UI64LIT(0x40000000000000))
        return false;
...
bool Unit::IsImmuneToSpellEffect(SpellEntry const* spellInfo, SpellEffectIndex index) const
{
    ...
    if(uint32 aura = spellInfo->EffectApplyAuraName[index])
    {
        ...
        // Bladestorm immunity // FG: is this still needed?
...
void Unit::ProcDamageAndSpellFor( bool isVictim, Unit * pTarget, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, SpellEntry const * procSpell, uint32 damage )
{
    if(!pTarget)
    {
        sLog.outError("-- Unit::ProcDamageAndSpellFor: typeid=%u lowguid=%u name=%s - pTarget=NULL!",GetTypeId(),GetGUIDLow(),GetName());
    }
    ...
    if (procFlag & MELEE_BASED_TRIGGER_MASK)
    {
        ...
        if (procExtra & (PROC_EX_CRITICAL_HIT|PROC_EX_PARRY|PROC_EX_DODGE|PROC_EX_BLOCK))
        {
            ...
            else //For attacker
            {
                ...
                if (procExtra&PROC_EX_DODGE && GetTypeId() == TYPEID_PLAYER && getClass() == CLASS_WARRIOR && pTarget)
                {
