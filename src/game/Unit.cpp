            ...
            if (dummySpell->SpellFamilyFlags & UI64LIT(0x10000000000000))
            {
                ...
                if (Group* pGroup = ((Player*)this)->GetGroup())
                {
                    ...
                    if (misdirect && misdirect != this)
                    {
                        ...
                        pVictim->AddThreat(misdirect, threat, false, procSpell ? SpellSchoolMask(procSpell->SchoolMask) : SPELL_SCHOOL_MASK_NORMAL, procSpell);
                        return true;
                    }
                }
                return false;
...
void Unit::SetInCombatState(bool PvP, Unit* enemy)
{
    ...
    {
        m_CombatTimer = 5000;
        // FG: if player enters combat by himself, remove "Inactive" buff
        RemoveAurasDueToSpell(43680);
        RemoveAurasDueToSpell(43681);
    }
...
void Unit::SendChangeCurrentVictimOpcode(HostileReference* pHostilReference)
{
    if (uint32 count = getThreatManager().getThreatList().size())
    {
...
void Unit::SendRemoveFromThreatListOpcode(HostileReference* pHostilReference)
{
