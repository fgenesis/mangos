...
bool LoginQueryHolder::Initialize()
{
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMYINFO,          "SELECT msg,forbidden FROM character_myinfo WHERE guid='%u'", m_guid.GetCounter());
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADEXTENDED,        "SELECT xp_multi_kill, xp_multi_quest FROM character_extra WHERE guid='%u'", m_guid.GetCounter());
...
class CharacterHandler
{
    public:
        ...
        void HandlePlayerBotLoginCallback(QueryResult * /*dummy*/, SqlQueryHolder * holder)
        {
            ...
            WorldSession *botSession = new WorldSession(lqh->GetAccountId(), NULL, SEC_PLAYER, masterSession->Expansion(), 0, 0, masterSession->GetSessionDbcLocale());
...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
    ...
    bool AllowTwoSideAccounts = sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_ACCOUNTS) || GetSecurity() > SEC_PLAYER; // FG: this is intended to be not GetSecurity() >= SEC_MODERATOR unlike everywhere else
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder *holder)
{
    ...
    // FG: load myinfo
    QueryResult *resultmyinfo = holder->GetResult(PLAYER_LOGIN_QUERY_LOADMYINFO);
    std::string myinf;
    if(resultmyinfo)
    {
    ...
    if (!pCurrChar->GetMap(true)->Add(pCurrChar))
    {
    ...
    {
        pCurrChar->RemoveAtLoginFlag(AT_LOGIN_LOW_HP, false);
