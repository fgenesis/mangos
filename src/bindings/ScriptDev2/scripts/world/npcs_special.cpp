...
struct MANGOS_DLL_DECL npc_mirror_imageAI : public ScriptedAI
{
    ...
    {
        m_unsummonTimer = 33000; // 3 extra secs for safety
        Reset();
    }

    uint32 m_uiCastTimer;
    uint32 m_unsummonTimer;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        {
            if (DoCastSpellIfCan(m_creature->getVictim(), (urand(0,100) < 50 ? SPELL_FROSTBOLT : SPELL_FIREBALL)) == CAST_OK)
                m_uiCastTimer = urand(3600, 4000);
        }
        else
            m_uiCastTimer -= uiDiff;

        if (m_unsummonTimer < uiDiff)
        {
            m_creature->CastSpell(m_creature, 7, true); // suicide
            m_unsummonTimer = 2000;
        }
        else
            m_unsummonTimer -= uiDiff;
    }
};
    ...
    void ReceiveEmote(Player* pPlayer, uint32 uiEmote)
    {
        ...
        if (uiEmote == TEXTEMOTE_KISS)
        {
            if (!pPlayer->HasAura(SPELL_MISTLETOE))
                m_creature->CastSpell(pPlayer,SPELL_MISTLETOE,false);
        }
    }
